...
from PyQt5.QtWidgets import QApplication, QWidget, QPushButton, QMessageBox, QTableWidget, QMainWindow
from PyQt5.QtCore import QCoreApplication
from PyQt5.QtWidgets import QWidget, QLabel, QVBoxLayout, QApplication, QToolTip
...
seat = [[1307, 480], [952, 479], [561, 480], [229, 482], [1188, 256], [943, 268], [611, 269], [350, 262], [1104, 24],
        [881, 124], [630, 126], [439, 34], [977, 37], [818, 39], [649, 44], [498, 48]]
seat_taken = []
maybe_seat_taken = []
...
class Detector(object):
    ...
    def image_detector(self, imnames, svnames, wait=0):
        ...
        for i in range(0, 568):  # --------------------------迭代次数
            ...
            ### show image start ###
            img_test = self.draw_result(image, result)  # 这个是预测出来的图片
...
#  座位检测的类，面向对象编程,继承自QWidget的类
class Seat_detection(QWidget):
    ...
    def UI(self):
        ...
        :return:
        """
        label = QLabel('欢迎来到座位管理系统', self)
        label.move(150, 50)
        label.setStyleSheet("QLabel{color:rgb(225,22,173,255);font-size:60px;font-weight:normal;font-family:Arial;}")

        QToolTip.setFont(QFont('SansSerif', 20))
        #  退出按钮
        qbtn = QPushButton('退出系统', self)
        qbtn.setToolTip('点击退出系统')
        qbtn.clicked.connect(QCoreApplication.instance().quit)
        qbtn.setStyleSheet("QPushButton{font-size:30px;}")
        qbtn.resize(200, 100)
        qbtn.move(10, 500)

        #  检测按钮
        detect = QPushButton('打开检测', self)
        detect.setToolTip('点击开始实时预测')
        detect.clicked.connect(self.control)
        detect.setStyleSheet("QPushButton{font-size:30px;}")
        detect.resize(200, 100)
        detect.move(10, 200)

        #  显示视频

        self.setGeometry(300, 300, 700, 700)  # 窗口的位置和大小
        self.setWindowTitle('基于深度学习的座位管理系统')  # 设置窗口的标题

        #   设置窗口背景图片
        palette1 = QPalette()
        palette1.setBrush(self.backgroundRole(), QBrush(QPixmap('./picture/AI.jpg')))
